...
def BFS(s):
    ...
    while not openList.empty():
        ...
        for (action, successor) in currentNode.state.successor_function():
            ...
            if child.state not in closedList and child.state not in [o.state for o in list(openList.queue)]:
...
def h2(node):
    ...
    for box_pos in node.state.box_positions:
        if node.state.target_positions:  
...
def a_star(start_state, h):
    ...
    while open:
        ...
        for action, successor_state in current_node.state.successor_function():
            ...
            in_close =child.state in [node.state for node in closed_set]
            ...
            if not in_close and not in_open:
                ...
                if in_open:
                    ...
                    for node in closed_set:
                        if child.state == node.state and child.f < node.f:
